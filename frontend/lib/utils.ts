...
export const addAccountFormSchema = z.object({
  ...
  initialDeposit: z.string()
    ...
    .refine((value) => !isNaN(value) && value >= 0, {
      message: "Initial deposit must be at least 0",
    }),
  nomineeName: z.string().min(1, "Nominee name is required"), // Required nominee name
  nomineeAge: z.string()
    .transform((value) => Number(value)) // Transform to a number
    .refine((value) => !isNaN(value) && value > 18, {
      message: "Nominee age must be a greater than 18",
    }),
  nomineeRelation: z.string().min(1, "Nominee relation is required"), // Required nominee relation
});
